...
describe('GymGeneralInformationComponent', () => {
  ...
  it('should display gym offers', () => {
    ...
    expect(compiled.querySelectorAll('.info-page').length).toBe(
      gymOffers.length
    );
    gymOffers.forEach((offer, index) => {
      expect(
        compiled.querySelectorAll('.info-page')[index].textContent
      ).toContain(offer.title);
      expect(
        compiled.querySelectorAll('.info-page')[index].textContent
      ).toContain(offer.description);
    });
  });
});
